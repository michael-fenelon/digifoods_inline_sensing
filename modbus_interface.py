        ...
        if self.connection:
            ...
            try:
                while True:
                    ...
                    result = self.client.write_register(address=0, value=10, device_id=1)
                    
                    result = self.client.read_input_registers(address=0, count=1, device_id=1)  # Get milliamps from motor 1                
                    print("result", result)

                    # Write a HIGH/LOW to the coil
                    result = self.client.write_coil(address=0, value=1, device_id=1)
                    result = self.client.write_coil(address=1, value=1, device_id=1)
