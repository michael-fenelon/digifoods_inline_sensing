...
class TSM():
    ...
    def gen_gui(self):
        self.gui_dict['VALVE']['valve_1'] = VALVE(window=self.canvas, color="light blue")
        self.gui_dict['VALVE']['valve_2'] = VALVE(window=self.canvas, color="light green")
        self.gui_dict['VALVE']['valve_3'] = VALVE(window=self.canvas, color="yellow")

        self.gui_dict['VALVE']['valve_1'].canvas.grid(row=0, column=0)
        self.gui_dict['VALVE']['valve_2'].canvas.grid(row=0, column=1)
        self.gui_dict['VALVE']['valve_3'].canvas.grid(row=2, column=2)
        #self.canvas.grid(row=0, column=0)   # Needs to be done by master (the module that creates an instance of TSM)
